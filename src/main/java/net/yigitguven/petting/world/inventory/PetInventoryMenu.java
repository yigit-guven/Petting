...
public class PetInventoryMenu extends AbstractContainerMenu {
    ...
    private final net.minecraft.world.SimpleContainer petInventory;
    ...
    public PetInventoryMenu(int id, Inventory playerInventory, Entity pet) {
        super(PettingModMenus.PET_INVENTORY, id);
        this.pet = pet;
        
        this.petInventory = new net.minecraft.world.SimpleContainer(1);
        if (pet != null) {
            if (((net.yigitguven.petting.IEntityData)pet).getPersistentData().contains("petting_saddle", 10)) {
                this.petInventory.setItem(0, net.minecraft.world.item.ItemStack.of(((net.yigitguven.petting.IEntityData)pet).getPersistentData().getCompound("petting_saddle")));
            }
            this.petInventory.addListener(container -> {
                if (!pet.level().isClientSide) {
                    net.minecraft.nbt.CompoundTag tag = new net.minecraft.nbt.CompoundTag();
                    container.getItem(0).save(tag);
                    ((net.yigitguven.petting.IEntityData)pet).getPersistentData().put("petting_saddle", tag);
                }
            });
        }

        // 1. Saddle Slot
        this.addSlot(new Slot(petInventory, 0, 142, 17) {
            @Override
