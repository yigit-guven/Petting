    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem event) {
        ...
        if (event.getItemStack().getItem() == whistleItem) {
            if (player.level() instanceof ServerLevel serverLevel) {
                ...
                if (summonedCount > 0) {
                    player.displayClientMessage(Component.literal("Â§aSummoned " + summonedCount + " pet(s) to your location!"), true);
                    serverLevel.playSound(null, player.blockPosition(), SoundEvents.ENDERMAN_TELEPORT, SoundSource.PLAYERS, 1.0F, 1.0F);
                } else {
                    player.displayClientMessage(Component.literal("Â§cYou have no custom pets in this dimension to summon."), true);
                }
            }
        }
    }
